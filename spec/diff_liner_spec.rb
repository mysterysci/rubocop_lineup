  ...
  context "calculates lines of uncommitted files" do
    it "single changed line" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.filenames).to eq ["a.txt"]
        expect(dl.file_line_changes["a.txt"]).to eq [1]
      end
    end
    ...
    it "two changed lines side-by-side" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.file_line_changes["a.txt"]).to eq [2, 3]
      end
    end
    ...
    it "two changed lines separated" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.file_line_changes["a.txt"]).to eq [2, 4]
      end
    end
    ...
    it "one line deleted" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.file_line_changes["a.txt"]).to eq []
      end
    end
    ...
    it "one line added" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.file_line_changes["a.txt"]).to eq [3]
      end
    end
    ...
    it "one add, one delete, 2 separate changes" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.file_line_changes["a.txt"]).to eq [1, 2, 6, 8]
      end
    end
    ...
    it "multiple files" do
      gf.make_temp_repo do |dir|
        ...
        dl = RubocopLineup::DiffLiner.diff_uncommitted(dir).strip_root!
        expect(dl.file_line_changes["a.txt"]).to eq [2, 4]
        expect(dl.file_line_changes["b.txt"]).to eq [1, 2]
      end
    end
  end
